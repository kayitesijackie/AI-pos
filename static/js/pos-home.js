        ...
        if (response.products === undefined || response.products.length === 0) {
            ...
            $.each(response.products, function (e, item) {
                trHTML += '<li class="list-group-item">\n' +
                    ...
                    '                                                                </div>' +
                    '                                                                <div class="col-auto">' +
                    '                                                                <span class="align-middle text-muted small float-right"> RWF ' + item.discount_price +
                    '                                                                </span>' +
                    '                                                                </div>' +
                    '                                                            </div>' +
                    '                                                        </div>' +
...
function loadTable(product_code, response) {
    ...
    $.ajax({
        ...
        success: function (data) {
            updateCartDetails('Total ' + data.cart_items_quantity + ' items, RWF ' +
            ...
            $.each(data.items.reverse(), function (e, item) {
                if (item.product.product_code === product_code) {
                    trHTML += `<tr class="clicked"><td><div class="text-left font-weight-bold">${item.product.name}</div></td><td>${item.product.weight}</td><td>RWF ${item.product.discount_price}</td><td class="text-center text-md-center"><span class="qty">${item.quantity} </span><div class="btn-group radio-group ml-2" data-toggle="buttons"><button type="button" data-product="1001" data-action="remove" class="btn btn-primary btn-sm mr-1 btn-rounded" onclick="updateUserOrder(${item.product.product_code}, 'remove')"><i class="fas fa-minus"></i></button><button type="button" data-product="1001" data-action="add" class="btn btn-primary btn-sm mr-1 btn-rounded" onclick="updateUserOrder(${item.product.product_code}, 'add')"><i class="fas fa-plus"></i></button></div></td><td class="font-weight-bold"><strong>RWF ${item.amount}</strong></td><td><button type="button" data-product="1001" data-action="add" class="btn btn-danger btn-sm mr-1 btn-rounded" onclick="updateUserOrder(${item.product.product_code}, 'delete')"><i class="fas fa-trash"></i></button></button></td></tr>`;
                } else {
                    trHTML += `<tr><td><div class="text-left font-weight-bold">${item.product.name}</div></td><td>${item.product.weight}</td><td>RWF ${item.product.discount_price}</td><td class="text-center text-md-center"><span class="qty">${item.quantity} </span><div class="btn-group radio-group ml-2" data-toggle="buttons"><button type="button" data-product="1001" data-action="remove" class="btn btn-primary btn-sm mr-1 btn-rounded" onclick="updateUserOrder(${item.product.product_code}, 'remove')"><i class="fas fa-minus"></i></button><button type="button" data-product="1001" data-action="add" class="btn btn-primary btn-sm mr-1 btn-rounded" onclick="updateUserOrder(${item.product.product_code}, 'add')"><i class="fas fa-plus"></i></button></div></td><td class="font-weight-bold"><strong>RWF ${item.amount}</strong></td><td><button type="button" data-product="1001" data-action="add" class="btn btn-danger btn-sm mr-1 btn-rounded" onclick="updateUserOrder(${item.product.product_code}, 'delete')"><i class="fas fa-trash"></i></button></button></td></tr>`;
                }
            });
...
function CalculateRefund(cash) {
    ...
    if (cash < cart_total_amount) {
        ...
        document.getElementById('RefundAmount').innerHTML = "Refund: RWF " + (cash - cart_total_amount).toString();
    } else {
